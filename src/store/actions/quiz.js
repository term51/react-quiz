// Асинхронная функция для получения данных с бека
...
export function fetchQuizById(quizId) {
   return async dispatch => {
      dispatch(fetchQuizesStart()); // работа с loading
...
export function quizAnswerClick(answerId) {
   return (dispatch, getState) => {
      const state = getState().quiz; // получить текущий state и поле quiz
      // проверка на нежелательные клики
      if (state.answerState) {
         const key = Object.keys(state.answerState)[0]; // получить ключ ответа
